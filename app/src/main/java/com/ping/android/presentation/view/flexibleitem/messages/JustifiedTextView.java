...
public class JustifiedTextView extends EmojiTextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!justifiedText.equals(getText().toString())) {
            ...
            //This class won't justify the text if the view width is 0
            //AND! won't justify the text if it's empty.
            if (viewWidth > 0 && !text.isEmpty()) {
    ...
    private boolean fitsInSentence(String word, List<String> sentence, boolean addSpaces) {
        ...
        float sentenceWidth = getPaint().measureText(getSameLength(stringSentence));

        return sentenceWidth < viewWidth;
    }

    private String getSameLength(String s) {
        ...
        while (end != BreakIterator.DONE) {
            String character = s.substring(start, end);
            if (end - start == 1) {
                randomStringBuilder.append(character);
            } else {
                randomStringBuilder.append("a");
            }
            start = end;
            end = it.next();
        }
